            ...
            <div class="row">
                        ...
                        <select class="form-control" name="idPropiedad" id="idPropiedad">
                            @foreach ($propiedades as $propiedad)
                                <option value="{{ $propiedad->id }}"
                                    {{ $propiedad->id == $propiedad->idPropiedad ? 'selected' : '' }}>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" name="idPropiedad" id="idPropiedad">
                            @foreach ($estadosPropiedad as $estadoPropiedad)
                                <option value="{{ $estadoPropiedad->id }}">
                                    {{ $estadoPropiedad->nombre }}</option>
                            @endforeach
                        </select>
                        @error('idPropiedad')
                            <div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
                        ...
                        <select class="form-control" name="idUsuario" id="idUsuario">
                            @foreach ($usuarios as $usuario)
                                <option value="{{ $usuario->id }}"
                                    {{ $usuario->id == $usuario->idUsuario ? 'selected' : '' }}>
                        ...
                        <select class="form-control" name="idUsuario" id="idUsuario">
                            @foreach ($tiposTransaccion as $tipoTransaccion)
                                <option value="{{ $tipoTransaccion->id }}"
                                    {{ $tipoTransaccion->id == $tipoTransaccion->idTipoTransaccion ? 'selected' : '' }}>
                ...
                <div class="col-xs-12 col-sm-12 col-md-12">
                    <div class="form-group">
                        ...
                        <input class="form-control" id="valor" type="text" name="valor"
                            value="{{ $transaccion->valor }}" class="form-control" placeholder="valor">
                        @error('valor')
                            <div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="col-xs-12 col-sm-12 col-md-12">
                    <div class="form-group">
                        ...
                        <textarea class="form-control" id="descripcion" name="descripcion" rows="4">{{ $transaccion->descripcion }}</textarea>
                        @error('descripcion')
                            <div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
                        @enderror
                    </div>
